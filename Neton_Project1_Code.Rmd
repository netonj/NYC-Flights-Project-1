...
```{r}
str(flights_joined$departure_delay) # check structure of data (will repeat for all variables below)
unique(flights_joined$departure_delay) # ensure no NA
```
...
flights_joined <- flights_joined %>% 
    filter(!is.na(late)) %>%  # last minute check for NA
...
```{r}
qqnorm(flights_joined$departure_delay) # plot normal distribution (will repeat for all variables)
qqline(flights_joined$departure_delay) # plot distribution line (will repeat for all variables)
```
...
# Prepare graphs for export
```{r}
  ggplot(flights_joined_reduced, aes(x = precipitation, y = departure_delay)) +
  geom_point(alpha = 0.3, color = "royalblue4") +
  geom_smooth(method = "lm", color = "red") +
  theme_classic() +
  ggtitle("Precipitation causes increase in Departure Delays",
          subtitle = "Heavier rainfall leads to increase in delay time") +
  labs(x = "Precipitation (in)", y = "Departure Delay (min)")
```
...
ggplot(flights_joined_reduced, aes(x = visibility, y = departure_delay)) +
  ...
  geom_smooth(method = "lm", color = "red") +
  theme_classic() +
  ggtitle("Precipitation causes marginal increase in Departure Delays",
          subtitle = "Higher visibility index leads to slight increase in delay time") +
  labs(x = "Visibility (mi)", y = "Departure Delay (min)")
```

```{r}
  precipitation <- ggplot(flights_joined_reduced, aes(x = precipitation, y = departure_delay)) +
  geom_point(alpha = 0.4, color = "royalblue4") +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  labs(
    title = "Heavier Rainfall Leads to Longer Departure Delays",
    subtitle = "Each point represents a flight; the line shows the overall trend",
    x = "Precipitation (in)",
...
```{r}
  visibility <- ggplot(flights_joined_reduced, aes(x = visibility, y = departure_delay)) +
  geom_point(alpha = 0.4, color = "royalblue4") +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  labs(
    title = "Heavier Visibility Leads to Slightly Longer Departure Delays",
    subtitle = "Each point represents a flight; the line shows the overall trend",
    x = "Visibility (mi)",
    y = "Departure Delay (min)"
  ) +
...
```{r}
visibility_bar <- flights_joined %>%
  mutate(visibility_bin = cut(visibility, breaks = c(0,2,4,6,8,10),
                              labels = c("0–2", "2–4", "4–6", "6–8", "8–10"))) %>%
  filter(!is.na(visibility_bin)) %>% 
  group_by(visibility_bin) %>%
  summarise(avg_delay = mean(departure_delay, na.rm = TRUE)) %>%
  ggplot(aes(x = visibility_bin, y = avg_delay, fill = visibility_bin)) +
  geom_col(fill = "royalblue4", show.legend = FALSE) +
  labs(
    title = "Visibility Has Little Effect on Departure Delays",
    subtitle = "Delays remain similar across visibility levels",
    x = "Visibility (miles)",
...
```{r}
ggsave(filename = "precipitation.png", plot = precipitation)
ggsave(filename = "visibility.png", plot = visibility)
ggsave(filename = "visibility_bar.png", plot = visibility_bar)
```
